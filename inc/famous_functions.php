...
function get_famous_quotes($categories=NULL,$sequence=NULL,$linkphrase=NULL,$multi=NULL,$timer=NULL,$noajax=NULL,$myoffset=0,$widgetid=NULL,$fullpage=NULL,$orderby='quoteID',$sort='ASC',$thisid=NULL,$disableaspect=NULL,$contributor=NULL) {
...
/*
	...
	$beforeAll =  utf8_decode($quotesoptions['famous_quotes_before_all']);
	$afterAll = utf8_decode($quotesoptions['famous_quotes_after_all']);
	$beforeloader = utf8_decode($quotesoptions['famous_before_loader']);
	$quoteloader = utf8_decode($quotesoptions['famous_loader']);
	$afterloader = utf8_decode($quotesoptions['famous_after_loader']);
	$famousajax = $quotesoptions['famous_ajax'];
	$loading =  utf8_decode($quotesoptions['famous_loading']);
	...
	//handle contributors
	$multiuser = $quotesoptions['famous_multiuser'];
	...
	if ($multi > 1){
		...
		$numrows = $wpdb->get_var("SELECT COUNT(`quoteID`) as 'rows' FROM " . WP_FAMOUS_QUOTES_TABLE . " WHERE visible='yes'" . $categoryquery . $userquery);
		...
		if ($fullpage){
			...
			if ($famousajax != 'Y' && $noajax != true)$offset=$myoffset;
		...
		. WP_FAMOUS_QUOTES_TABLE . "` WHERE `visible`='yes'" . $categoryquery . $userquery
	...
	*/
	...
	if ( !empty($result) ){	
		...
		if ($famousajax != 'Y' && $noajax != true ) {
			...
			if (!$quoteloader && $timer < 2) $output .= '<div class="famous_quote-'.$widgetid.'" '.$event.' >';
			else  $output .= '<div class="famous_quote-'.$widgetid.'">';
		}
		...
		if ($multi > 1){
			...
			$output .= $beforeAll.'<ul>';
			foreach ( $result as $get_one )
				$output .= '<li>'.famous_output_one($get_one,$multi,$disableaspect).'</li>';	
			$output .= '</ul>'.$afterAll;
		...
		//output if one quote only
			...
			//if a specific quote
			if ( $thisid && $thisid != '' )$output .= famous_output_one($specificresult,$multi,$disableaspect);
			...
			//if it is not a specific quote
				...
				$output .= famous_output_one($get_one,$multi,$disableaspect);
			}
...
function famous_random_quote($categories='all',$sequence=false,$linkphrase='',$noajax=false,$multi=1,$timer=0,$orderby='quoteID',$sort='ASC',$disableaspect=NULL, $contributor=NULL) {
	echo get_famous_quotes($categories,$sequence,$linkphrase,$multi,$timer,$noajax,0,'',false,$orderby,$sort,'',$disableaspect, $contributor);	
}

//this is a TEMPLATE TAG. It ECHOES a specific quote.
function famous_a_quote($id=1,$linkphrase='',$noajax=false,$disableaspect=NULL) {
	echo get_famous_quotes('',true,$linkphrase,'','',$noajax,'','','','','',$id,$disableaspect);
}

//this is a SHORTCODE [famous-random]
function famous_random_shortcode($atts, $content=NULL) {
	...
	return get_famous_quotes($categories,$sequence,$linkphrase,$multi,$timer,$noajax,$offset,$widgetid,$fullpage,'quoteID','ASC','',$disableaspect, $user);
}

//this is a SHORTCODE [famous-all]
function famous_all_shortcode($atts, $content=NULL) {
	...
	return get_famous_quotes($categories,$sequence,$linkphrase,$rows,$timer,$noajax,$offset,$widgetid,$fullpage,$orderby,$sort,'',$disableaspect, $user);
}

//this is a SHORTCODE [famous-id]
function famous_id_shortcode($atts, $content=NULL) {
	...
	return get_famous_quotes('',true,$linkphrase,'','',$noajax,'','','','','',$id,$disableaspect);
}

//this FORMATS a given quote according to the settings
function famous_output_one($get_one,$multi=NULL,$disableaspect=NULL) {
	...
	if(!$disableaspect || $disableaspect == '' || $disableaspect == false){
		...
		$beforeQuote = utf8_decode($quotesoptions['famous_quotes_before_quote']);
		$afterQuote = utf8_decode($quotesoptions['famous_quotes_after_quote']);
		$beforeAuthor = utf8_decode($quotesoptions['famous_quotes_before_author']);
		$afterAuthor = utf8_decode($quotesoptions['famous_quotes_after_author']);
		$beforeSource = utf8_decode($quotesoptions['famous_quotes_before_source']);
		$afterSource = utf8_decode($quotesoptions['famous_quotes_after_source']);
		$linkto = utf8_decode($quotesoptions['famous_quotes_linkto']);
		$sourcelinkto = utf8_decode($quotesoptions['famous_quotes_sourcelinkto']);
		$sourcespaces = utf8_decode($quotesoptions['famous_quotes_sourcespaces']);	
		$authorspaces = utf8_decode($quotesoptions['famous_quotes_authorspaces']);
		$ifnoauthor = utf8_decode($quotesoptions['famous_if_no_author']);	
	} else {
		$beforeAuthor = ' ';
		$ifnoauthor = ' ';
		$beforeSource = ' ';	
	...
	$putQuotesFirst = utf8_decode($quotesoptions['famous_quotes_put_quotes_first']);
	$output = '';	
...
function make_categories($user=NULL) {
	...
	$quotesoptions = array();
	$quotesoptions = get_option('famous_quotes_options');
	$defaultcategory = $quotesoptions['famous_default_category'];
	
	if($user)$where = "` WHERE `user`='".$user."'";
	$allcategories = $wpdb->get_col("SELECT `category` FROM `" . WP_FAMOUS_QUOTES_TABLE . $where );
...
function mostused($field) {
	...
	$sql = "SELECT `".$field."` FROM `" . WP_FAMOUS_QUOTES_TABLE . "` WHERE `".$field."` IS NOT NULL AND `".$field."` !='' AND `user`='".$current_user->user_nicename."'" ;
...
function wp_quotes_random() {return famous_random_quote();}
function wp_quotes($id) {return famous_a_quote($id);}
function wp_quotes_page($data) {return famous_all_shortcode();}
